...
public partial class UriDal : IControllerDal
{
    ...
    public IEnumerable<string> GetPlanList(string filter = null, bool isRegexFilter = true)
    {
        ...
        {
            return cloudUriHandler.ListFilesInFolder(_planPath).GetAwaiter().GetResult();
        }
        else
        {
            if( !isRegexFilter )
            ...
            return cloudUriHandler.ListFilesInFolder( _planPath ).GetAwaiter().GetResult()?.Where( f => regex.IsMatch( f ) )?
                .Select( f =>  f  );
        }
    }
    ...
    public IEnumerable<long> GetPlanInstanceIdList(string planUniqueName)
    {
        ...
        IEnumerable<string> files = cloudUriHandler.ListFilesInFolder( _planPath ).GetAwaiter().GetResult()
            .Select( f =>  f  );
    ...
    public Plan GetPlan(string planUniqueName)
    {
        ...
        string planFile = Synapse.Services.Controller.Dal.Utilities.PathCombine( _planPath, $"{planUniqueName}.yaml" );
        return SynapseUtilities.YamlHelpers.DeserializeFile<Plan>( planFile );
    }

    public Plan CreatePlanInstance(string planUniqueName)
    {
       string planContent = cloudUriHandler.GetFileInFolder( _planPath, $"{planUniqueName}.yaml" ).GetAwaiter().GetResult();
        Plan plan = SynapseUtilities.YamlHelpers.Deserialize<Plan>(planContent);
    ...
    {
        string planFile = Synapse.Services.Controller.Dal.Utilities.PathCombine( _histPath, $"{planUniqueName}_{planInstanceId}.yaml" );
